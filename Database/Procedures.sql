...
BEGIN
	DELETE FROM Library_orders where Library_orders.orderId=Id;
END$$
DELIMITER ;
...
DELIMITER ;
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `getUser`(IN username VARCHAR(100))
BEGIN
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `promote`(IN UserId INT)
BEGIN
	UPDATE user set Privilege=true where user.UserId=UserId;
END$$
DELIMITER ;
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `placeOrder`(IN ISBN INT, Publisher VARCHAR(100), quantity INT)
BEGIN
	INSERT INTO Library_Orders VALUES(default,ISBN,Publisher,quantity);
END$$
DELIMITER ;
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `getUserID`(IN username VARCHAR(100))
BEGIN
